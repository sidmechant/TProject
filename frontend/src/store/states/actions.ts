...
import { AnimationStates, GameAction, GameContext, MapTheme } from "../../types/machine";
...
export const changeCurrentAction: GameAction<'changeCurrent'> = (context, _) => {
	...
	return {
		starsColor: palettes[player.mapInfo.id], current: player
	};
};
...
export const scoreGameAction: GameAction<'score'> = (context, event) => {
	return {
		...
		players: context.players.map((p, i) => {
			...
			context.physic?.setPower(i, false);
			p.ulti = false;
			context.physic?.setUlti(i, false);
...
export const startPhysicAction = (context: GameContext): Partial<GameContext> => {
	(context.animation === 'Ulti') && context.physic?.play();
	const i: number | undefined = (context.animation !== 'Ulti') ? context.physic?.start() : undefined;
	// const i: number | undefined =  undefined;
	return { animation: undefined, current: (i !== undefined) ? context.players[i] : context.current };
};

export const setIsMapVisibleAction: GameAction<'setIsMapVisible'> = () => {
	return { isMapVisible: true };
};
