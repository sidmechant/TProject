...
export function createWorld({
	...
	const step = () => {
		...
		const isDestroy = (): Player['powerMedieval'][] => {
			...
			for (const stone of playerNeg.ultiMedieval.stones) {
				if (stone.isDestroy) { console.log(playerNeg.ultiMedieval.stones.indexOf(stone)); destroy.push(stone)};
			}
	...
	const handlePaddleCollision = (paddle: P2.Body) => {
		...
		(paddle === playerNeg.body) && (playerNeg.powerNinja.speedFactor = 1);
		(paddle === playerPos.body) && (playerPos.powerNinja.speedFactor = 1);
