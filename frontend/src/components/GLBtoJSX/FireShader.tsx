...
import { SSR, DepthOfField, Bloom, Noise, Glitch, Vignette, EffectComposer, Selection, Select, SelectiveBloom, Outline } from '@react-three/postprocessing'
...
const FireShader = ({
  ...
  const { size } = useThree();
  const lol = useRef<THREE.Mesh>(null!);
  ...
  return (
    ...
    <ambientLight ref={lightref} intensity={2} />
      ...
      {/* <EffectComposer autoClear={false}> //fais tout planter quand je desactive reactive la visibilité de la map
        <SelectiveBloom 
        lights={[lightref]}
        selection={meshRef} 
