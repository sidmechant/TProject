...
export function RotatingMesh({ children }: rotatingMenuProps) {
    ...
    const colorScale = chroma.scale(['#e9765b', '#5F8C9E', '#f2a490', '#879e75', '#e9765b']).mode('lch');
    ...
    useEffect(() => {
        camera.rotation.z = MathUtils.degToRad(5); // rotation de 15 degrés
    }, [camera]);
    ...
    useEffect(() => {
        ...
        gl.domElement.addEventListener('wheel', handleWheelEvent);
