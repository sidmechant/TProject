...
export default function StarsDisappearEffect({ visible, location, colors }: StarsDisappearProps) {
    ...
    useEffect(() => {
        if (visible && starsRef.current) {
            setSize(0);
            starsRef.current.position.set(6.4 * location, 0, 1);
    ...
    return <>
        <group ref={starsRef}>
            <Stars quantity={15} maps={star2} color={colors[0]} size={size} duration={Math.random() * 0.4 + 0.1} />
            <Stars quantity={10} maps={star2} color={colors[1]} size={size} duration={Math.random() * 0.4 + 0.1} />
            <Stars quantity={15} maps={star1} color={colors[2]} size={size} duration={Math.random() * 0.4 + 0.1} />
            <Stars quantity={10} maps={star1} color={colors[3]} size={size} duration={Math.random() * 0.4 + 0.1} />
        </group>
    </>
};
